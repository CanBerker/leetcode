...
var threeSum = function(nums) {
    ...
    return threeSumFoundOnline(nums);
};
        ...
        for (let j = 0; j < nums.length; j++) {
            for (let k = 0; k < nums.length; k++) {
                if (i !== j && j !== k && i !== k && nums[i] + nums[j] + nums[k] === 0) {
                    let foundTriplet = [nums[i], nums[j], nums[k]].sort();
                    console.log(foundTriplet)
                    let foundTripletStr = foundTriplet.join('');
                    if (!found.has(foundTripletStr)) {
                        found.add(foundTripletStr);
                        result.push(foundTriplet);
                    }
                }
            }
        }
...
var threeSumFoundOnline = function(nums) {
	const results = []
